  ...
  useEffect(() => {
    const loadExam = async () => {
      ...
      try {
        ...
        setAcademicYear(examData.academicYear);
        setStartDate(new Date(examData.startDate));
        setEndDate(new Date(examData.endDate));
        
        // Set selected classes based on exam's selected sections
        const classIds = Array.from(new Set(examData.selectedSections.map(s => s.grade)));
        setSelectedClasses(classIds);
        setSelectAllClasses(classIds.length === allClassIds.length);
        
        // Set slots
        if (examData.slots && examData.slots.length > 0) {
          setSlots(examData.slots);
        } else {
          // Default slot if no slots exist
          setSlots([{
            id: '1',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const examData = {
        ...
        status: exam?.status || 'draft' as const,
        ...
        isFinalised: exam?.isFinalised || false,
        description: `${name.trim()} for ${academicYear}`
      };

      await examAPI.updateExam(id!, examData as any);
      ...
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            The exam you're trying to edit doesn't exist.
          </p>
      ...
      <div className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end space-x-2">
            <Button type="button" variant="outline" onClick={() => navigate(`/exams/${id}`)}>Cancel</Button>
